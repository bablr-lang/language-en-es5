...
import { get, parseObject, printSource } from '@bablr/agast-helpers/tree';
...
export { startsIdentifierPattern };
...
export const reservedWords = freezeRecord([
  'arguments',
  'await',
  'break',
  'case',
...
let reservedWords_ = new Set(arrayValues(reservedWords));
  ...
  *Expression({ props: { power }, getState }) {
    ...
    if (!s.shifted) {
      if ((res = yield eatMatch(m`<ParenthesisExpression '(' />`))) {
        ...
        (res = yield eatMatch(
          m`<UnaryExpression ${printStreamSource(
