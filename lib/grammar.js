...
const atrivial = class ES5Grammar extends es3.grammar.atrivial {
  ...
  *Property({ s }) {
    let { held } = s();
    yield eatHeld(m`key$: <_ObjectKey />`);
    let firstToken = printSource(BTree.getAt(1, getRoot(held).value.bounds[0]).property.value.node);
    let isIndex = /\d/.test(firstToken[0]);

    let cn =
      isIndex || reservedWords_.has(printSource(held))
  ...
  *Method({ s }) {
    ...
    while (!co.done) {
      ...
      if (refName === 'name') {
        if (s().held) {
          co.advance(yield eatHeld(m`name$: <_ObjectKey />`));
        } else {
          co.advance(yield eat(m`name$: <_ObjectKey />`));
        }
